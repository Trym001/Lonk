...
if __name__ == '__main__':
    ...
    q_recv = Queue()
    lock = Lock()
    p = Process(target=send_data, args=(dev1, dev2, dev3, q))
    p2 = Process(target=recv_data, args=(q_recv,))
    #p3 = Process(target=drive_with_cmd, args=(q_recv,))
    # ------------------------------------------------------------
    p.start()
    p2.start()
    #p3.start()
